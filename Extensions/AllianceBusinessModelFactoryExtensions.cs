...
namespace FenixAlliance.ABM.Hub.Extensions
{
    ...
    public static class FactoryExtensions
    {
        ...
        public static IServiceCollection AddAllianceBusinessModelContextFactory<TContext>(this IServiceCollection collection, Action<DbContextOptionsBuilder> optionsAction = null, ServiceLifetime contextAndOptionsLifetime = ServiceLifetime.Singleton) where TContext : DbContext
        {
            // instantiate with the correctly scoped provider
            collection.Add(new ServiceDescriptor(typeof(IDbFactoryService<TContext>),sp => new DbContextFactoryService<TContext>(sp),contextAndOptionsLifetime));

            // dynamically run the builder on each request
            collection.Add(new ServiceDescriptor(typeof(DbContextOptions<TContext>),sp => GetOptions<TContext>(optionsAction, sp), contextAndOptionsLifetime));
        ...
        public static async Task EnsureDbCreatedAndSeedWithCountOfAsync(this DbContextOptions<ABMContext> options, int count)
        {
            var builder = new DbContextOptionsBuilder<ABMContext>(options);
            builder.UseLoggerFactory(new LoggerFactory());
            using var context = new ABMContext(builder.Options);
